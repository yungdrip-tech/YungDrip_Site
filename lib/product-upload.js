...
import { PRODUCT_UPLOAD_PREFIX } from "@/lib/product-upload-url";

export { PRODUCT_UPLOAD_PREFIX };
...
export async function readProductImage(filename) {
  ...
  await connectToDatabase();
  const record = await ProductImage.findOne({ filename: safeFilename }).select("data contentType");
