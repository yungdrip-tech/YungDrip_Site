...
export async function createPasswordResetToken(email) {
  ...
  const normalizedEmail = sanitizeEmail(email);
  const user = await User.findOne({ email: normalizedEmail }).select("+resetTokenHash +resetTokenExpiresAt");
  ...
  const token = crypto.randomBytes(32).toString("hex");
  user.resetTokenHash = digestToken(token);
  user.resetTokenExpiresAt = new Date(Date.now() + RESET_TOKEN_DURATION_MS);
  await user.save();
...
export async function resetPasswordWithToken({ token, password }) {
  ...
  const user = await User.findOne({
    resetTokenHash: digestToken(token),
