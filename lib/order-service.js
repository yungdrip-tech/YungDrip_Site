...
async function buildOrderItems(cartItems) {
  ...
  const requestedByProductSize = new Map();
  ...
  for (const [key, requestedQuantity] of requestedByProductSize.entries()) {
    ...
    if (product.outOfStock) {
      throw new HttpError(400, `${product.name} is currently out of stock`);
    }

    const availableStock = getStockForSize(product, size) ?? 0;

    if (requestedQuantity > availableStock) {
      throw new HttpError(
        400,
        `${product.name} (${size}) only has ${availableStock} left in stock`
      );
    }
  }
...
async function decrementStockForOrderItems(items) {
  ...
  for (const item of items) {
    const key = `${item.productId}:${item.size}`;
    quantityByProductSize.set(key, (quantityByProductSize.get(key) || 0) + item.quantity);
  }

  for (const [key, quantity] of quantityByProductSize.entries()) {
    const [productId, size] = key.split(":");
    ...
    if (!updatedProduct) {
      const legacyUpdate = await Product.findOneAndUpdate(
      ...
      if (!legacyUpdate) {
        throw new HttpError(400, "One or more items are no longer in stock");
      }
    }
  }
}
...
export async function finalizeCheckoutOrder({ userId, orderId, razorpayPaymentId, razorpayOrderId, razorpaySignature }) {
  ...
  sendOrderConfirmationEmail(normalizedOrder).catch(() => {});
...
export async function updateOrderStatusAsAdmin(orderId, nextStatus) {
  ...
  if (["processing", "packed", "shipped", "delivered", "cancelled"].includes(nextStatus)) {
    sendOrderStatusEmail(normalizedOrder, nextStatus).catch(() => {});
  }
